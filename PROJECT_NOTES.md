...
### 편집 모드 토글
- 일정표 위에 슬라이드 토글 스위치(오른쪽 끝 배치). 원이 왼쪽=보기 전용(기본), 오른쪽=편집 모드.
- **보기 전용 상태에서 비활성화되는 것**: 일정 드래그/리사이즈, 빈 칸 클릭으로 폼 열기, 후보 카드 드래그-드롭, Delete/Ctrl+C/V 단축키, 상단 지도 터치(스크롤 중 실수 방지용 오버레이).
- 상단 지도 iframe 위에 투명 오버레이(`#mapTouchBlock`)를 올려 보기 전용 시 스크롤 중 지도가 실수로 조작되는 문제 해결.
- JS 상태: `let editMode=false;` / 함수: `setEditMode(on)`

### 상단 지도
- 구글 "내 지도(My Maps)" 임베드(MYMAPS_MID). "전체 핀 보기", "저장한 방문지 목록 열기" 버튼.
- iframe은 `.map-iframe-wrap`(relative)으로 감싸고, `#mapTouchBlock`(absolute 오버레이)으로 편집 모드 외 터치 차단.

### 좌측 사이드바 — SPA 페이지 전환 방식
- 사이드바 항목 클릭 시 팝업 대신 **메인 영역 전체가 해당 섹션 페이지로 전환**됨(홈페이지처럼 이동).
- JS 상태: `let currentPage='home';` / 함수: `navigateTo(key)`
  - `key='home'` → `#pageHome`(일정표 영역) 표시, `#pageSection` 숨김
  - `key='visit'|'lodging'|'food'|'shopping'|'history'` → `#pageHome` 숨김, `#pageSection` 표시
- `#pageSection` 구성: `← 일정으로` 버튼 + 섹션 제목(아이콘+이름) + `＋ 항목 추가` 버튼 + 설명 + 카드 그리드(`#sectionCards`)
- 카드 클릭 → 기존 개별 상세 모달(`detailViewOverlay`) 그대로 사용(수정·삭제 포함).
- 사이드바 활성 항목 하이라이트: `data-page` 속성 + `.sidenav-item.active` 클래스.
- Firebase 실시간 리스너도 `currentPage` 기준으로 `renderDetailCards()` 호출.
- `renderDetailCards()`는 `#sectionCards`를 타겟으로 렌더링.
- 주의: `#pageSection`은 반드시 `.main-col` 안에 있어야 함(바깥에 두면 flex overflow 발생).

### 하단 후보/숙소 카드 (홈 화면)
- 방문지 / 식당 / 숙소는 **사이드바 상세보기와 하단 카드가 같은 데이터**.
- 후보 카드를 일정표로 드래그 → 일정 등록(편집 모드에서만 동작).
...
- [ ] 섹션 페이지에서 카드 정렬/필터 기능
- [ ] 모바일에서 섹션 페이지 카드 레이아웃 최적화
